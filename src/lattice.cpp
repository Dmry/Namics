#include "lattice.h"
Lattice::Lattice(vector<Input *> In_, string name_) : BC(6) // resize the boundary condition vector to 6 for Mesodyn
{															//this file contains switch (gradients). In this way we keep all the lattice issues in one file!
	if (debug)
		cout << "Lattice constructor" << endl;
	In = In_;
	name = name_;
...
{
	if (debug)
		cout << "AllocateMemory in lattice " << endl;
	Real r, VL, LS;
	Real rlow, rhigh;
	...
	if (fjc == 1)
	{
		if (gradients < 3)
		{
			...
			Zero(L, M);
			lambda_1 = (Real *)malloc(M * sizeof(Real));
			Zero(lambda_1, M);
			lambda1 = (Real *)malloc(M * sizeof(Real));
			Zero(lambda1, M);
			lambda0 = (Real *)malloc(M * sizeof(Real));
			Zero(lambda0, M);
		}
	}
	else
		...
		{ //fjc != 1;
			...
			if (geometry == "cylindrical")
			{
				...
				for (i = fjc; i < M - fjc; i++)
				{
					...
					VL = L[i] / PIE * fjc;
					if ((rlow - r) * 2 + r > 0.0)
						LAMBDA[i] += 0.5 / (1.0 * FJC - 1.0) * 2.0 * rlow / VL;
					if ((rhigh - r) * 2 + r < MX)
					...
					for (j = 1; j < fjc; j++)
					{
						...
						if ((rhigh - r) * 2 + r < offset_first_layer + MX)
...
bool Lattice::PutM()
{
	...
	case 1:
		...
		if (geometry == "spherical")
		{
			volume = 4 / 3 * PIE * (pow(MX + offset_first_layer, 3) - pow(offset_first_layer, 3));
		}
		if (geometry == "cylindrical")
		{
...
bool Lattice::CheckInput(int start)
{
	...
	if (GetValue("bondlength").size() > 0)
	{
		...
		if (bond_length < 1e-10 || bond_length > 1e-8)
		{
			cout << " bondlength out of range 1e-10..1e-8 " << endl;
			success = false;
		}
	}
	...
	lambda = 0;
	lattice_type = "";
	options.push_back("simple_cubic");
	options.push_back("FCC");
	options.push_back("hexagonal");
	Value = GetValue("lattice_type");
	if (Value.length() > 0)
	{
		if (!In[0]->Get_string(Value, lattice_type, options, "Input for 'lattice_type' not recognized. 'Simple_cubic': lambda=1/6; Z=6; FCC: lambda=1/3; Z=3; 'hexagonal': lambda=1/4; Z=4;"))
			success = false;
		else
		{
	...
	else
	{
		if (lambda > 0)
		{
			if (GetValue("lambda").size() > 0)
				cout << "'lattice_type' has been set: 'lambda'-value is ignored" << endl;
			if (GetValue("Z").size() > 0)
				cout << "'lattice_type' has been set: 'Z' (coordination number) is ignored" << endl;
		}
		else
		{
			if (GetValue("lambda").size() > 0)
			{
				...
				if (lambda < 0.0 || lambda > 1.0 / 3.0)
				{
					lattice_type = "simple_cubic";
					lambda = 1.0 / 6.0;
					cout << "'lambda'-value out of bounds 0..1/3. Default 1/6 is used instead" << endl;
				}
				else
				{
					if (lambda > 0.33 && lambda < 0.34)
					{
						lattice_type = "FCC";
						lambda = 1.0 / 3.0;
					}
					if (lambda > 0.16 && lambda < 0.167)
					{
						lattice_type = "simple_cubic";
						lambda = 1.0 / 6.0;
					}
					if (lambda > 0.24 && lambda < 0.26)
					{
	...
	case 2:
		...
		if (BC[2] == "periodic" || BC[3] == "periodic")
		{
			...
			{
				success = false;
				cout << "For boundaries in y-direction: 'periodic' BC should be set to upper and lower bounds " << endl;
			}
		}
		break;
	case 3:
...
Real Lattice::Moment(Real *X, int n)
{
	Real Result = 0;
	Real cor;
	if (gradients != 1 || geometry != "planar")
	{
		cout << "Moments analysis can only be done on one-gradient, planar system " << endl;
		return Result;
	}
	remove_bounds(X);
	for (int i = fjc; i < M; i++)
...
{
	if (debug)
		cout << "PutProfiles in lattice " << endl;
	int x, y, z, i;
	int length = X.size();
	switch (gradients)
	{
	case 1:
		for (x = 0; x < M; x++)
		{
			if (fjc == 1)
				...
				fprintf(pf, "%e\t", 1.0 * (x + 1) / fjc - 1.0);
			for (i = 0; i < length; i++)
				fprintf(pf, "%.20g\t", X[i][x]);
			fprintf(pf, "\n");
		}
		break;
	case 2:
...
void Lattice::PushOutput()
{
	...
	if (offset_first_layer > 0)
		push("offset_first_layer", offset_first_layer);
	push("volume", volume);
	...
	case 3:
		push("n_layers_z", MZ);
		if (BZ1 == 1)
			push("lowerbound_z", mirror);
		if (BZM == MZ - 1)
			push("upperbound_z", mirror);

		if (BX1 == MX)
			push("lowerbound_x", periodic);
		if (BXM == 1)
			push("upperbound_x", periodic);
		if (BY1 == MY)
			push("lowerbound_y", periodic);
		if (BYM == 1)
			push("upperbound_y", periodic);
		if (BZ1 == MZ)
			push("lowerbound_z", periodic);
		if (BZM == 1)
			push("upperbound_z", periodic);
		// Fall through
	case 2:
		push("n_layers_y", MY);
		if (BY1 == 1)
			push("lowerbound_x", mirror);
		if (BYM == MY - 1)
			push("upperbound_x", mirror);
		// Fall through
	case 1:
		push("n_layers", MX);
		if (BX1 == 1)
			push("lowerbound", mirror);
		if (BXM == MX - 1)
			push("upperbound", mirror);
		break;
	default:
		break;
	}
...
void Lattice::Side(Real *X_side, Real *X, int M)
{ //this procedure should use the lambda's according to 'lattice_type'-, 'lambda'- or 'Z'-info;
	if (debug)
		cout << " Side in lattice " << endl;
	Real *SIDE;
	int j, k;
	Zero(X_side, M);
	switch (gradients)
	...
	case 2:
		...
		else
		{
			...
			{
				cout << "Warning: In FCC the function side is not correctly implemented" << endl;
				SIDE = new Real[M];
				Zero(SIDE, M);
				Add(SIDE, X + 1, M - 1);
				Add(SIDE + 1, X, M - 1);
...
{ //this procedure should function on simple cubic lattice.
	if (debug)
		cout << " propagate in lattice " << endl;
...
{
	if (debug)
		cout << " remove_bounds (Real) in lattice " << endl;
	int x, y;
	int j;
	switch (gradients)
	{
...
{
	if (debug)
		cout << "set_bounds (Reals) in lattice " << endl;
	int x, y;
	int j;
	switch (gradients)
	{
	...
	case 2:
		...
		if (X[0] > 0)
			X[0] = sqrt(X[0]);
		X[(MX + 1) * JX] = X[MX * JX] * X[(MX + 1) * JX + 1];
		if (X[(MX + 1) * JX] > 0)
			X[(MX + 1) * JX] = sqrt(X[(MX + 1) * JX]);
		X[0 + MY + 1] = X[JX + MY + 1] * X[0 + MY];
		if (X[MY + 1] > 0)
			X[MY + 1] = sqrt(X[MY + 1]);
		X[(MX + 1) * JX + MY + 1] = X[MX * JX + MY + 1] * X[(MX + 1) * JX + MY];
		if (X[(MX + 1) * JX + MY + 1] > 0)
			X[(MX + 1) * JX + MY + 1] = sqrt(X[(MX + 1) * JX + MY + 1]);
		break;
	case 3:
		if (sub_box_on != 0)
		{
			...
			else
			{
				for (size_t i = 0; i < coor.size(); ++i)
				{
					...
					case 3:
						if (xyz.size() != 3)
						{
							cout << "In mon " + seg_name + " pinned_range  the expected 'triple coordinate' -with brackets- structure '(x,y,z)' was not found. " << endl;
							success = false;
							break;
						}
						...
						if (px < 1 || px > MX)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the x-coordinate in pinned_range out of bounds: 1.." << MX << endl;
							success = false;
						}
						py = In[0]->Get_int(xyz[1], 0);
						if (py < 1 || py > MY)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the y-coordinate in pinned_range out of bounds: 1.." << MY << endl;
							success = false;
						}
						pz = In[0]->Get_int(xyz[2], 0);
						if (pz < 1 || pz > MZ)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the y-coordinate in pinned_range out of bounds: 1.." << MZ << endl;
							success = false;
						}
					...
					case 2:
						if (xyz.size() != 2)
						{
							cout << "In mon " + seg_name + " pinned_range  the expected 'pair of coordinate' -with brackets- structure '(x,y)' was not found. " << endl;
							success = false;
						}
						else
						{
							...
							if (px < 0 || px > MX + 1)
							{
								cout << "In mon " + seg_name + ", for 'pos' " << i << ", the x-coordinate in pinned_range out of bounds: 0.." << MX + 1 << endl;
								success = false;
							}
							py = In[0]->Get_int(xyz[1], 0);
							if (py < 0 || py > MY + 1)
							{
								cout << "In mon " + seg_name + ", for 'pos' " << i << ", the y-coordinate in pinned_range out of bounds: 0.." << MY + 1 << endl;
								success = false;
							}
						}
						H_p[i] = px * JX + py;
					...
					case 1:
						if (xyz.size() != 1)
						{
							cout << "In mon " + seg_name + " pinned_range  the expected 'single coordinate' -with brackets- structure '(x)' was not found. " << endl;
							success = false;
						}
						else
						{
...
bool Lattice::ReadRangeFile(string filename, int *H_p, int &n_pos, string seg_name, string range_type)
{
	...
	case 1:
		if (length == MX)
		{ //expect to read 'mask file';
			if (n_pos == 0)
			{
				for (i = 0; i < length; ++i)
				{
					if (In[0]->Get_int(lines[i], 0) == 1)
						n_pos++;
				}
				if (n_pos == 0)
				{
					cout << "Warning: Input file for locations of 'particles' does not contain any unities." << endl;
				}
			}
			else
			{
		...
		{ //expect to read x only
			...
			else
			{
				while (i < length)
				{
					...
					if (length_xyz != 1)
					{
						cout << "In mon " + seg_name + " " + range_type + "_filename  the expected 'single coordinate' 'x' was not found. " << endl;
						success = false;
					}
					else
					{
						...
						if (px < 1 || px > MX)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the x-coordinate in " + range_type + "_filename out of bounds: 1.." << MX << endl;
							success = false;
						}
					}
					H_p[i] = px;
		...
		{ //expect to read x,y
			...
			else
			{
				while (i < length)
				{
					...
					if (length_xyz != 2)
					{
						cout << "In mon " + seg_name + " " + range_type + "_filename  the expected 'pair of coordinates' 'x,y' was not found. " << endl;
						success = false;
					}
					else
					{
						...
						if (py < 1 || py > MY)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the y-coordinate in " + range_type + "_filename out of bounds: 1.." << MY << endl;
							success = false;
						}
					}
					H_p[i] = px * JX + py;
		...
		{ //expect to read x,y,z
			...
			else
			{
				while (i < length)
				{
					...
					if (length_xyz != 3)
					{
						cout << "In mon " + seg_name + " " + range_type + "_filename  the expected 'triple coordinate' structure 'x,y,z' was not found. " << endl;
						success = false;
					}
					else
					{
						...
						if (pz < 1 || pz > MZ)
						{
							cout << "In mon " + seg_name + ", for 'pos' " << i << ", the y-coordinate in " + range_type + "_filename out of bounds: 1.." << MZ << endl;
							success = false;
						}
					}
					H_p[i] = px * JX + py * JY + pz;
...
bool Lattice::FillMask(int *Mask, vector<int> px, vector<int> py, vector<int> pz, string filename)
{
	...
	case 1:
		if (readfile)
		{
			...
			{
				success = false;
				cout << "inputfile for filling delta_range has not expected length in x-direction" << endl;
			}
			else
			{
				for (int x = 1; x < MX + 1; x++)
					Mask[x] = In[0]->Get_int(lines[x], -1);
			}
		}
		else
		{
	...
	case 2:
		...
		else
		{
			for (int i = 0; i < length_px; i++)
			{
				...
				{
					success = false;
					cout << " y-value in delta_range out of bounds; " << endl;
				}
				if (success)
					Mask[px[i] * JX + py[i]] = 1;
			}
	...
	case 3:
		if (readfile)
		{
			...
			{
				success = false;
				cout << "inputfile for filling delta_range has not expected length in x,y,z-directions" << endl;
			}
			else
			{
				for (int x = 1; x < MX + 1; x++)
					for (int y = 1; y < MY + 1; y++)
						for (int z = 1; z < MZ + 1; z++)
							Mask[x * JX + y * JY + z] = In[0]->Get_int(lines[x * JX + y * JY + z], -1);
			}
		}
		else
		{
...
{
	if (debug)
		cout << "CreateMask for lattice " + name << endl;
	bool success = true;
	H_Zero(H_MASK, M);
	if (block)
	{
...
{
	if (debug)
		cout << "GenerateGuess in lattice " << endl;
...
{
	if (debug)
		cout << "StoreGuess in output" << endl;
	bool success = true;
...
void Lattice::UpdatePsi(Real *g, Real *psi, Real *q, Real *eps, int *Mask)
{ //not only update psi but also g (from newton).
	int x, y;
#ifndef CUDA
	int z;
#endif
	...
	case 1:
		...
		for (x = 1; x < M - 1; x++)
		{
			...
			if (Mask[x] == 0)
			{ //use updated psi at subsequent coordinates = Jacobian trick. Upwind to larger x.
				if (geometry == "planar")
				{
		...
		for (x = M - 2; x > 0; x--)
		{ //perform backward update as well in case potentials need to 'diffuse' to lower x: downwind.
			epsXplus = epsXmin;
				...
				for (z = 1; z < MZ + 1; z++)
				{
					...
					if (Mask[x * JX + y * JY + z] == 0)
						psi[x * JX + y * JY + z] = (epsXmin * psi[(x - 1) * JX + y * JY + z] + epsXplus * psi[(x + 1) * JX + y * JY + z] +
													...
													C * q[x * JX + y * JY + z]) /
												   (epsXmin + epsXplus + epsYmin + epsYplus + epsZmin + epsZplus);
				}
			}
		}
		for (x = MX; x > 0; x--)
...
void Lattice::UpdateQ(Real *g, Real *psi, Real *q, Real *eps, int *Mask)
{ //Not only update q (charge), but also g (from newton).
	int x, y;
#ifndef CUDA
	int z;
#endif
				...
				if (Mask[x * JX + y] == 1)
				{
					if (geometry == "flat")
					{
						q[x * JX + y] = (epsXmin * psi[(x - 1) * JX + y] + epsXplus * psi[(x + 1) * JX + y] +
										 ...
										 (epsXmin + epsXplus + epsYmin + epsYplus) * psi[x * JX + y]) /
										C;
					}
					else
					{
						q[x * JX + y] = (lambda_1[x * JX + y] * epsXmin * psi[(x - 1) * JX + y] + lambda1[x * JX + y] * epsXplus * psi[(x + 1) * JX + y] +
										 epsYmin * psi[x * JX + y - 1] + epsYplus * psi[x * JX + y + 1] -
										 (lambda_1[x * JX + y] * epsXmin + lambda1[x * JX + y] * epsXplus + epsYmin + epsYplus) * psi[x * JX + y]) /
										C;
					}
					g[x * JX + y] = -q[x * JX + y];
				}
				...
				for (z = 1; z < MZ; z++)
				{
					...
					if (Mask[x * JX + y * JY + z] == 1)
					{
						psi[x * JX + y * JY + z] = (epsXmin * psi[(x - 1) * JX + y * JY + z] + epsXplus * psi[(x + 1) * JX + y * JY + z] +
													epsYmin * psi[x * JX + (y - 1) * JY + z] + epsYplus * psi[x * JX + (y + 1) * JY + z] +
													epsZmin * psi[x * JX + y * JY + z - 1] + epsZplus * psi[x * JX + y * JY + z + 1] -
													(epsXmin + epsXplus + epsYmin + epsYplus + epsZmin + epsZplus) * psi[x * JX + y * JY + z]) /
												   C;
